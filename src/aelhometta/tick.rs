            ...
            if let Some(ctrl) = self.controllers.get(&cuid) {
                ...
                if let Some(enuid) = ctrl.exec_optuid {
                    if self.nodes.contains_key(&enuid) {
                        ...
                        // Execute Command, ignore other variants of Content at execution
                        match econtent {
                            Content::Command(command) => {
                                use self::Command::*;
                                ctrl.flags.success = false; // "pessimistic default"

                                if self.commandswitch(command.to_u8().unwrap()) {
                                    match command {
                                        Abs => {
                                    ...
                                    ctrl.flags.success = true; // "success" of Space, substituted for command
                                }

                                // Count even if it has been NOPped
